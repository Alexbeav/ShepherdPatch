...
std::string DisplayMouseInput(std::string_view value)
{
    ...
    if (input == "BUTTON_2")
        return "MMB";
    if (input.starts_with("BUTTON_"))
        return "Mouse " + input.substr(7);
...
std::string DisplayKeyboardScanCode(std::uint8_t scanCode)
{
    static const std::unordered_map<std::uint8_t, std::string_view> names{
        ...
        {0x9C, "Numpad Enter"}, {0x9D, "Right Ctrl"}, {0xB8, "Right Alt"},
...
std::uint32_t ResolveGenericPcActionCommand(std::uint32_t commandId,
                                            std::string_view resourceName)
{
    ...
    constexpr std::uint32_t kPrimaryWorldActionCommand = 5;
    return commandId == 34 || commandId == 46 ? kPrimaryWorldActionCommand : commandId;
}
} // namespace shh
