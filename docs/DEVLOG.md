...
Cubic found that the first publication gate used an unbounded spin. A stalled worker can therefore stop device creation forever.
...
Cubic marked this finding as addressed. The final Cubic status passed at commit `bcc6693`.
...
Cubic passed both follow-up requests at their final heads. Both requests have no unaddressed automated-review finding.
